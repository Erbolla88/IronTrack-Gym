...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col md:flex-row font-sans selection:bg-yellow-400 selection:text-indigo-900">
      {/* Sidebar */}
      <nav className="w-full md:w-72 bg-indigo-600 flex flex-col h-screen sticky top-0 text-white shadow-2xl">
          ...
          {routines.map((routine) => (
            ...
            >
              ...
              <button 
                ...
                className={`p-2 rounded-lg transition-all opacity-0 group-hover:opacity-100 hover:bg-white/10 hover:text-red-400`}
              >
        ...
        <div className="p-6 bg-indigo-700/50 m-4 rounded-[32px] border border-indigo-400/20">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-indigo-400 overflow-hidden border-2 border-white/20">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[10px] font-bold text-indigo-300 uppercase truncate">Operador: {user.uid.slice(0, 8)}</span>
            </div>
          </div>
        </div>
      </nav>
...
function RoutineView({ routine, user, customMedia, onImageClick }: { routine: Routine, user: User, customMedia: Record<string, string>, onImageClick: (url: string) => void }) {
  ...
  return (
    <div className="h-full flex flex-col p-8 md:p-12 max-w-6xl mx-auto space-y-12">
      {/* Header */}
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-8 py-4">
        <div className="space-y-2">
          ...
          <h2 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter uppercase leading-none">{routine.name}</h2>
          <div className="flex items-center gap-8 pt-4">
            <div className="flex items-center gap-3 text-slate-400">
              <Calendar className="w-5 h-5" />
              <span className="text-xs font-bold uppercase tracking-widest">Sem {routine.weekStart}</span>
            </div>
          </div>
        </div>
        <div className="flex flex-col items-end gap-2">
           <div className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 flex items-center gap-6">
              <div className="text-right">
                <div className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-1">Carga Semanal</div>
                <div className="text-2xl font-black text-slate-800 leading-none">OPTIMIZADO</div>
              </div>
              <div className="w-16 h-16 bg-indigo-50 border-4 border-white rounded-[20px] shadow-sm flex items-center justify-center -rotate-6">
                 <Dumbbell className="w-8 h-8 text-indigo-600 rotate-6" />
              </div>
           </div>
        </div>
      </header>
      ...
      <div className="flex-1 min-h-0 pb-12">
        <motion.div 
          ...
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-white rounded-[48px] shadow-md border border-slate-100 flex flex-col h-full overflow-hidden"
        >
          {activeDay ? (
            <ExerciseManager 
            ...
            <div className="p-24 text-center text-slate-200 font-black text-4xl uppercase tracking-tighter">Datos Fuera de Línea</div>
          )}
        </motion.div>
      </div>
    </div>
    ...
    <div className="flex flex-col h-full">
      <div className="p-10 border-b border-slate-50 flex items-center justify-between flex-wrap gap-6">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h4 className="font-black uppercase tracking-widest text-lg text-slate-800 leading-none mb-1">Matriz de Protocolo</h4>
            <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">{exercises.length} Unidades Activas</p>
          </div>
        </div>
        <button 
          onClick={() => setShowAddForm(true)}
          className="flex items-center gap-3 px-8 py-4 bg-indigo-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all hover:-translate-y-1 active:translate-y-0"
        >
      ...
      <div className="flex-1 overflow-y-auto p-10 space-y-4 no-scrollbar">
        <div className="grid grid-cols-[60px_100px_1fr_120px_120px_100px] px-8 text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-4">
          <span>ORDEN</span>
          <span>VISUAL</span>
        ...
        <AnimatePresence initial={false}>
          {exercises.map((ex, idx) => (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, x: 20 }}
              className="group grid grid-cols-[60px_100px_1fr_120px_120px_100px] items-center p-6 rounded-[32px] bg-slate-50/50 hover:bg-white border border-transparent hover:border-slate-100 hover:shadow-xl hover:shadow-slate-200/50 transition-all mb-4 relative overflow-visible"
            >
              <span className="text-sm font-black text-slate-200">{String(idx + 1).padStart(2, '0')}</span>
              <div 
                className="w-20 h-20 rounded-2xl bg-white overflow-hidden shadow-sm flex items-center justify-center p-0 border border-slate-100 group-hover:scale-105 transition-transform cursor-zoom-in"
              ...
              <div className="px-6">
                <span className="font-black tracking-tight text-xl uppercase truncate block text-slate-900">{ex.name}</span>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                  <span className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest">Base de Datos en Tiempo Real</span>
                </div>
              </div>
              <div className="flex flex-col items-center justify-center p-4 bg-white rounded-2xl shadow-sm border border-slate-100">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 leading-none">Volumen</span>
                <span className="text-lg font-black text-indigo-600">{ex.sets} x {ex.reps}</span>
              </div>
              <div className="flex flex-col items-center justify-center p-4 bg-indigo-600 rounded-2xl shadow-lg shadow-indigo-100 ml-4">
                <span className="text-[9px] font-black text-indigo-300 uppercase tracking-widest mb-1 leading-none">Intensidad</span>
                <span className="text-lg font-black text-white">{ex.weight} KG</span>
              </div>
              <div className="flex justify-end relative z-[100] pointer-events-auto">
                ...
                >
                  {deletingId === ex.id ? (
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap">¿CONFIRMAR?</span>
                  ) : (
        ...
        {exercises.length === 0 && !showAddForm && (
          <div className="h-full flex flex-col items-center justify-center py-20 bg-slate-50/50 rounded-[40px] border-2 border-dashed border-slate-100">
            <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center mb-6 text-slate-100">
              <PlusCircle className="w-10 h-10" />
            </div>
            <span className="text-xs font-black text-slate-300 uppercase tracking-widest">Matriz Vacía • Esperando Entrada</span>
          </div>
        )}
        ...
        <AnimatePresence>
          {showAddForm && (
            <motion.div 
              ...
              className="p-10 rounded-[40px] bg-indigo-900 text-white space-y-8 shadow-2xl relative overflow-hidden"
            >
              <div className="relative z-10 grid grid-cols-1 md:grid-cols-[1.5fr_1fr_1fr_1fr] gap-6">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em]">Designación</label>
                  <input 
                    autoFocus
                    type="text" 
                    ...
                    className="w-full bg-indigo-800 border-2 border-indigo-700 p-5 rounded-2xl text-white font-black uppercase tracking-widest focus:border-yellow-400 outline-none placeholder:text-indigo-400/50"
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em]">Series</label>
                  <input 
                    type="number" 
                    value={newExercise.sets}
                    onChange={(e) => setNewExercise({...newExercise, sets: Number(e.target.value)})}
                    className="w-full bg-indigo-800 border-2 border-indigo-700 p-5 rounded-2xl text-white font-black tracking-widest focus:border-yellow-400 outline-none"
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em]">Reps</label>
                  <input 
                    type="number" 
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em]">Carga (KG)</label>
                  <input 
                    type="number" 
              ...
              <div className="relative z-10 flex gap-4 pt-4">
                <button 
                  ...
                  className="flex-1 py-6 bg-yellow-400 text-indigo-900 font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-white transition-all shadow-xl shadow-yellow-900/20 active:scale-95"
                >
                  Desplegar Unidad
                </button>
                <button 
                  onClick={() => setShowAddForm(false)}
                  className="px-8 py-6 bg-indigo-800 text-white font-bold uppercase tracking-widest rounded-2xl hover:bg-indigo-700 transition-all border border-indigo-700"
                >
                  Abortar
                </button>
              </div>
